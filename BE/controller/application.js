...
const updateProjectStatus = async (req, res) => {
  try {
    const { projectId, closed } = req.body;
    if (!projectId || typeof closed !== "boolean") {
      return res.status(400).json({
        message: "Project ID is required, and closed must be a boolean",
        status: false,
      });
    }
    ...
    if (projects.length > 0) {
      const updateProjectQuery = `
        ...
        SET closed = ?
        WHERE projectId = ?`;
      await pool.query(updateProjectQuery, [closed, projectId]);
    } else {
      const checkStudentProjectQuery = `
        SELECT * FROM studentProject
        WHERE projectId = ?`;
      ...
      if (studentProjects.length > 0) {
        const updateStudentProjectQuery = `
          ...
          SET closed = ?
          WHERE projectId = ?`;
        await pool.query(updateStudentProjectQuery, [closed, projectId]);
      } else {
        res.status(404).json({
